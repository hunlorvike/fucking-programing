...
Trong **Entity Framework (EF)**, **Data Loading** đề cập đến cách thức tải dữ liệu liên quan đến các thực thể (entities)
có quan hệ với nhau. Việc tải dữ liệu đúng cách ảnh hưởng trực tiếp đến hiệu suất của ứng dụng, đặc biệt khi làm việc
với các bảng có quan hệ phức tạp. EF cung cấp ba phương pháp chính để tải dữ liệu:
...
**Eager Loading** là phương pháp tải dữ liệu ngay lập tức khi thực hiện truy vấn. Điều này có nghĩa là khi bạn truy vấn
một thực thể, các thực thể liên quan (như các bảng con trong quan hệ 1-n hoặc n-n) sẽ được tải đồng thời với thực thể
chính trong cùng một truy vấn.
...
EF sẽ thực hiện một truy vấn SQL với `JOIN` để lấy tất cả dữ liệu của đơn hàng, khách hàng và các mục đơn hàng trong một
lần.
...
**Lazy Loading** là phương pháp tải dữ liệu khi cần, tức là khi bạn truy cập một thuộc tính của thực thể mà thuộc tính
đó chưa được tải. Điều này giúp giảm bớt khối lượng dữ liệu tải về ban đầu. Tuy nhiên, điều này có thể dẫn đến nhiều
...
- **N+1 Query Problem**: Nếu bạn truy vấn nhiều thực thể, có thể tạo ra nhiều truy vấn con, gây giảm hiệu suất. Ví dụ:
  Truy vấn tất cả các đơn hàng, sau đó tải khách hàng cho từng đơn hàng, dẫn đến N+1 truy vấn.
...
**Explicit Loading** là phương pháp tải dữ liệu liên quan khi bạn yêu cầu rõ ràng. Đây là sự kết hợp giữa Eager Loading
và Lazy Loading, cho phép bạn tải dữ liệu theo yêu cầu mà không phải đợi nó tự động được tải khi truy cập vào thuộc
tính.
...
Trong Entity Framework, các thực thể (**entities**) là các lớp đại diện cho các bảng trong cơ sở dữ liệu. Mỗi lớp Entity
thường có các thuộc tính tương ứng với các cột trong bảng. Để minh họa các phương pháp **Eager Loading**, **Lazy Loading
** và **Explicit Loading**, ta sẽ tạo ra một số lớp Entity cơ bản như sau:
...
Các phương pháp Data Loading trong Entity Framework – **Eager Loading**, **Lazy Loading**, và **Explicit Loading** – đều
có những ưu điểm và nhược điểm riêng. Việc lựa chọn phương pháp nào phụ thuộc vào yêu cầu ứng dụng và cách bạn muốn kiểm
soát việc tải dữ liệu.
...
- **Explicit Loading** là sự kết hợp giữa Eager và Lazy, cho phép bạn tải dữ liệu khi cần thiết mà không làm ảnh hưởng
  đến hiệu suất quá nhiều.
